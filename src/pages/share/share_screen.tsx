...
export default function ShareScreen() {
    ...
    const sheetId = localStorage.getItem("currentSheet");
    ...
    async function addCollaborators(spreadsheetId: string, email: string) {
        if (!useGetToken()) {
            setmessage({
                status: "ERROR",
                message: "Please login to add a collaborator",
            });
            return;
        }
            ...
            {useGetToken() ? (
                <div className="bg-neutral-50 px-4 w-5/12 h-fit flex flex-col overflow-hidden rounded-3xl py-8">
                    ...
                    <div className="flex flex-row items-center gap-1 ">
                        ...
                        <button
                            className="border-2 border-yellow-400 bg-white hover:bg-yellow-200 h-12 pr-2 pl-1 rounded-tr-full rounded-br-full"
                            onClick={() => addCollaborators(sheetId!, email)}
                        >
                    ...
                    {message.status === "NONE" && (
                        <p className="pl-3 mt-2 text-gray-800 text-sm">
                            Add a collaborator to share the spreadsheet
                        </p>
                    )}
                    {message.status === "ERROR" && (
                        <p className="pl-3 mt-2 text-red-500 text-sm">
                            {message.message}
                        </p>
                    )}
                    {message.status === "SUCCESS" && (
                        <p className="pl-3 mt-2 text-green-500 text-sm">
                            {message.message}
                        </p>
                    )}
